...
__global__ void onda2d(float *u, float *u_m1, float *u_p1,float dt,float dx,float c,int T,int Nx, int Ny,int t) {
	...
	__shared__ float s_u_p1[1024];
	__shared__ float s_u[1024];
	__shared__ float s_u_m1[1024];
	
	s_u_m1[y * blockDim.x + x]= u_m1[y * blockDim.x + x];
	s_u[y * blockDim.x + x]= u[t*Nx*Ny+y * blockDim.x + x];
    ...
    if (x>0 && x<Nx-1 && y>0 && y<Ny-1){
            	...
            	s_u_p1[y*Ny+x] = dt*dt/(dx*dx)*c*c*(s_u[y*Ny+x+1]+s_u[y*Ny+x-1]+
                s_u[(y+1)*Ny+x]+s_u[(y-1)*Ny+x]-4.0*s_u[y*Ny+x]) + 2.0*s_u[y*Ny+x] - s_u_m1[y*Ny+x];
            
		u_m1[y*Ny+x]=s_u[y*Ny+x];

            	u[(t+1)*Nx*Ny+y*Ny+x]=s_u_p1[y*Ny+x];
