...
__global__ void reset(cufftComplex *f1, int N){
...
//variables para la transfomada de fourier.
   ...
   cufftHandle plan[size_time];
   cufftPlan2d(plan,N_grilla,N_grilla,CUFFT_C2C);
   ...
   printf("%f \n", dt);
    ...
    for (int t=0;t<size_time;t++){
        ...
        real_complejo<<<grid,bloque>>>(cuda_rho,f_dc,N_grilla);
        cudaDeviceSynchronize();

        cufftExecC2C(plan[t], f_dc,ft_d,CUFFT_FORWARD);
        cudaDeviceSynchronize(); 
       ...
       // poisson<<<grid,bloque>>>(ft_d,ft_d_k,k_d,N_grilla,100,(float) L_size/N_grilla);
        ...
        cufftExecC2C(plan[t],ft_d_k,u_dc,CUFFT_INVERSE);
        cudaDeviceSynchronize();
